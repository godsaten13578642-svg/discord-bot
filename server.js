...
// The Render blueprint generates MC_API_KEY and SETUP.md tells you to paste
// it into the plugin — so it must be authoritative when set.
...
const mcAuth = (req, res, next) => {
  ...
  if (!isValidMcKey(key)) return res.status(403).json({ error: 'Invalid API key' });
  next();
};

// Accept either the first-server key or the global/env key — the MC_API_KEY
// env var (generated by the Render blueprint) is the deployment truth.
function isValidMcKey(key) {
  return !!key && (key === features.mcApiKey || key === getServerFeatures('_global').mcApiKey);
}
...
wss.on('connection', (ws, req) => {
  ...
  if (!isValidMcKey(apiKey)) { ws.close(1008, 'Unauthorized'); return; }
...
// Render free services sleep after ~15 min idle; when they wake, outbound
// connections (Discord bot, Minecraft WebSocket) need to re-establish. These
// handlers tell everyone what happened instead of leaving silent gaps.
...
client.once(Events.ClientReady, (c) => {
  ...
  // Channel for spin-down / wake-up notices (free-tier API restarts).
  // Accepted forms: raw channel ID, or "guildId:channelId" to restrict the
  // notice to one server. Leave empty to disable notices.
  ...
  // If the Minecraft WebSocket reattached before Discord finished logging in
  // (common right after a wake-up), announce it now.
  if (global.mcWsClients.size > 0) {
    sendStatusNotice('🟢 **API is back online** after a spin-down — the Minecraft server reconnected automatically.');
  }
  updateBotPresence();
